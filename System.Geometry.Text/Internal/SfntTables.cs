...
namespace System.Geometry.Text
{
    ...
    static class SfntTables
    {
        public static uint[] ReadTTCHeader(BinaryReader reader)
        {
        ...
        public static TableRecord[] ReadFaceHeader(BinaryReader reader)
        {
        ...
        public static void ReadHead(BinaryReader reader, TableRecord[] tables, out FaceHeader header)
        {
        ...
        public static void ReadMaxp(BinaryReader reader, TableRecord[] tables, ref FaceHeader header)
        {
        ...
        public static MetricsHeader ReadMetricsHeader(BinaryReader reader)
        {
            // skip over version
        ...
        public static MetricsEntry[] ReadMetricsTable(BinaryReader reader, int glyphCount, int metricCount)
        {
        ...
        public static OS2Data ReadOS2(BinaryReader reader, TableRecord[] tables)
        {
        ...
        public static void ReadPost(BinaryReader reader, TableRecord[] tables, ref FaceHeader header)
        {
        ...
        public static void ReadLoca(BinaryReader reader, TableRecord[] tables, IndexFormat format, uint[] table, int offset, int count)
        {
            ...
            if (format == IndexFormat.Short)
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    table[offset++] = (uint)(reader.ReadUInt16BE() << 1);
                }
            }
            else
            {
                for (int i = 0; i < count; i++)
                {
                    table[offset++] = reader.ReadUInt32BE();
                }
            }
        }

        public static NameData ReadNames(BinaryReader reader, TableRecord[] tables)
        {
            ...
            var stringData = new StringData[count];
        ...
        public static int[] ReadCvt(BinaryReader reader, TableRecord[] tables)
        {
        ...
        public static byte[] ReadProgram(BinaryReader reader, TableRecord[] tables, FourCC tag)
        {
        ...
        public static bool SeekToTable(BinaryReader reader, TableRecord[] tables, FourCC tag, bool required = false)
        {
        ...
        public static void ReadGlyph(BinaryReader reader, int glyphIndex, int recursionDepth, GlyphDescription[] glyphTable, uint glyfOffset, uint glyfLength, uint[] loca, int off)
        {
            ...
            else if (header.ContourCount == -1)
            {
                ...
                for (int i = 0; i < subglyphs.Length; i++)
                    ReadGlyph(reader, subglyphs[i].Index, recursionDepth + 1, glyphTable, glyfOffset, glyfLength, loca, 0);
            ...
            else
            {
                ...
                glyphTable[glyphIndex] = new SimpleGlyphDescription
                {
                    Points = new Point[0],
        ...
        static SimpleGlyphDescription ReadSimpleGlyph(BinaryReader reader, int contourCount)
        {
            ...
            for (int i = 0; i < points.Length; i++)
            {
                ...
                y += delta;
                points[i].Y = y;
                points[i].Type = (f & SimpleGlyphFlags.Line) != 0 ? PointType.Line : PointType.Quadratic;
            }

            return new SimpleGlyphDescription
            {
                Points = points,
        ...
        static CompositeGlyphDescription ReadCompositeGlyph(BinaryReader reader)
        {
            ...
            var result = new CompositeGlyphDescription { SubGlyphs = subglyphs.ToArray() };
        ...
        static string ExtractString(BinaryReader reader, uint baseOffset, StringData data)
        {
